...
export class UserPageLayerService {
    ...
    constructor(private _http: Http, private configuration: Configuration) {
        this.actionUrl = configuration.serverWithApiUrl + 'userpagelayer/';
    ...
    public Add = (UserPageLayer: UserPageLayer): Observable<UserPageLayer> => {
        return this._http.post(this.actionUrl + 'create', JSON.stringify(UserPageLayer), {headers: this.headers})
            .map((response: Response) => <UserPageLayer>response.json())
            .catch(this.handleError);
    }
